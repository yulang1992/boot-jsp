...
function loadData() {
	var options = {
			...
			loadTableUrl: basePath + "notice/getNoticeList.do",
			pager:"#pager"
	};
	...
	var JqGridColModel=[
						...
						{name:'content', index:'content', width:300, align:'left', sorttype:'string', hidden:true,sortable:false}
	                ];
	loadtable();
 ...
 */
$("#add").click(function(){
	document.location.href = basePath + "notice/viewNotice.do";
});
...
$("#edit").click(function(){
	...
	if(selectRow.length > 1){
		...
		document.location.href = basePath + "notice/viewNotice.do?id=" + selectRow[0];
	}else {
...
$("#del").click(function(){
	...
	if(selectRows.length > 0){
		swal({
			...
			$.ajax({
				url: basePath + 'notice/delNotice.do',
				type: 'post',
				dataType: 'json',
				traditional: true,
				data: {
