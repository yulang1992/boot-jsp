...
function loadData() {
	var options = {
			...
			loadTableUrl: basePath + "user/getUserList.do",
			pager:"#pager"
	};
	...
	var colNames = ['ID','用户名','姓名','手机','创建时间','是否启用'];
	var JqGridColModel=[
						...
						{name:'realName', index:'realName', width:200, align:'left', sorttype:'string', sortable:false},
						{name:'mobilePhone', index:'mobilePhone', width:200, align:'left', sorttype:'string', sortable:false},
						{name:'createTime', index:'createTime', width:200, align:'left', formatter:formatDate, sortable:false},
	...
	//格式时间
	function formatDate(cellvalue, options, rowObjec){
		return $.myTime.UnixToDate(cellvalue, true);
	}
}
 ...
 */
$("#add").click(function(){
	document.location.href = basePath + "user/viewUser.do";
});
...
$("#edit").click(function(){
	...
	if(selectRow.length > 1){
		...
		document.location.href = basePath + "user/viewUser.do?id=" + selectRow[0];
	}else {
...
$("#enabled").click(function(){
	...
	if(selectRows.length == 0){
		...
		$.ajax({
			url: basePath + 'user/isEnabled.do',
			type: 'post',
			dataType: 'json',
			traditional: true,
			data: {
